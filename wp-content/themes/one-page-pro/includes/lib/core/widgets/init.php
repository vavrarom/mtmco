...
class Layers_Widgets {
	...
	*/
	...
	public function __construct() {
		...
		$widget_dir = '/includes/lib/core/widgets/';
		$module_dir = '/includes/lib/core/widgets/modules/';
	...
	*/
	...
	public static function register_builder_sidebar( $post_id = 0, $post_title = '' ) {
		register_sidebar( array(
			...
			'name'		=> $post_title . __( ' Body' , 'one-page' ),
	...
	public function register_dynamic_sidebars(){
		...
		foreach( $dynamic_sidebars as $dynamic_sidebar ){
			register_sidebar( array(
							...
							'description'	=> __( 'Layers Builder section.' , 'one-page' ),
	...
	public function add_revision_fields( $fields ) {
		...
		if( !( is_object( $current_screen) && 'revision' == $current_screen->base && !isset( $_GET[ 'action' ] ) ) ) {
			$fields['post_content_filtered'] = __( 'Raw Page Data', 'one-page' );
		}

		$new_fields = array();
		foreach( $fields as $f_key => $f_value ){
			if( $f_key == 'post_content' ){
				$new_fields['_layers_widget_order'] = __( 'Widget Order', 'one-page' );
			}
	...
	*/
	...
	public function admin_enqueue_scripts(){
		...
		wp_register_script(
			...
			get_template_directory_uri() . '/includes/lib/core/widgets/js/slider.js' ,
			array(),
			LAYERS_VERSION,
			true
		);
		wp_localize_script( LAYERS_THEME_SLUG . '-admin-slider-widget' , 'sliderwidgeti18n', array(
			'confirm_message' => __( 'Are you sure you want to remove this slide?' , 'one-page' )
		) );

		// Content Widget
		wp_register_script(
			LAYERS_THEME_SLUG . '-admin-content-widget' ,
			get_template_directory_uri() . '/includes/lib/core/widgets/js/content.js' ,
			array(),
			LAYERS_VERSION,
			true
		);
		wp_localize_script( LAYERS_THEME_SLUG . '-admin-content-widget' , 'contentwidgeti18n', array(
			'confirm_message' => __( 'Are you sure you want to remove this column?' , 'one-page' )
		) );

		// Repeatable Widget
		wp_register_script(
			LAYERS_THEME_SLUG . '-admin-repeater-widget' ,
			get_template_directory_uri() . '/includes/lib/core/widgets/js/repeater.js' ,
			array(),
			LAYERS_VERSION,
			true
		);
		...
		wp_register_script(
			...
			get_template_directory_uri() . '/includes/lib/core/widgets/js/tinymce.js' ,
			array(
				'editor',
				'word-count',
				'quicktags',
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/includes/lib/core/widgets/js/widget-accordians.js' ,
			array(
				LAYERS_THEME_SLUG . '-admin-slider-widget',
				LAYERS_THEME_SLUG . '-admin-content-widget',
				'backbone',
...
if( !function_exists( 'layers_backup_page_sidebars_widgets' ) ) {
	function layers_backup_page_sidebars_widgets( $page = NULL, $no_revisions = FALSE ){
		...
		if( !class_exists( 'Layers_Widget_Migrator' ) ) {
			require get_template_directory() . '/includes/lib/core/helpers/migrator.php';
		}
