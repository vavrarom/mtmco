	...
	*/
	...
	public function get_sort_options(){
		...
		$this->sort_options = array(
			'updated-desc' => array(
				'label' => __( 'Last Updated' , 'one-page' ),
				'excerpt-label' => __( 'by last updated' , 'one-page' ),
			),
			'id-desc' => array(
				'label' => __( 'Newest to Oldest' , 'one-page' ),
				'excerpt-label' => __( 'by release date' , 'one-page' ),
			),
			'name-asc' => array(
				'label' => __( 'Item Name A - Z' , 'one-page' ),
				'excerpt-label' => __( 'alphabetically' , 'one-page' ),
			),
			'sales-desc' => array(
				'label' => __( 'Best Sellers' , 'one-page' ),
				'excerpt-label' => __( 'by highest sales' , 'one-page' ),
			),
			'rating-desc' => array(
				'label' => __( 'Best Rated' , 'one-page' ),
				'excerpt-label' => __( 'by highest rating' , 'one-page' ),
			),
			'price-asc' => array(
			...
			'trending-desc' => array(
				'label' => __( 'Trending Items' , 'one-page' ),
	...
	private function do_envato_api_call( $endpoint = 'market/total-items.json', $query_string = NULL , $method = 'get', $timeout = 5 ){
		...
		if( is_wp_error( $remote_query ) ){
			...
			return new WP_Error( __( 'Error' , 'one-page' ) , __( 'Something broke and we can\'t load the stream' , 'one-page' ) );
		}
	}
