...
class Layers_Form_Elements {
	...
	*/
	...
	public function get_sort_options( $options = array() ){
		$options[ json_encode( array( 'orderby' => 'date', 'order' => 'desc' ) ) ] = __( 'Newest First' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'date', 'order' => 'asc' ) ) ] = __( 'Oldest First' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'rand', 'order' => 'desc' ) ) ] = __( 'Random' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'title', 'order' => 'asc' ) ) ] = __( 'Titles A-Z' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'title', 'order' => 'desc' ) ) ] = __( 'Titles Z-A' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'comment_count', 'order' => 'desc' ) ) ] = __( 'Most Comments' , 'one-page' );
		$options[ json_encode( array( 'orderby' => 'menu_order', 'order' => 'desc' ) ) ] = __( 'Custom Order' , 'one-page' );
		return $options;
	}
		...
		switch( $input->type ) {
			...
			*/
			case 'rte' :
				...
				$allow_buttons = ( isset( $input->allow_buttons ) && is_array( $input->allow_buttons ) ? $input->allow_buttons : array('bold','italic','underline','align','insertImage','insertVideo','createLink','insertOrderedList','undo','html' ) );
			...
			case 'image' :
				...
				<section class="layers-image-container <?php echo ( $has_image ) ? 'layers-has-image' : '' ; ?>">
					<div class="layers-image-display layers-image-upload-button">
						...
						<?php } ?>
						<!-- Remove button -->
						<a href="#removeimage" class="layers-image-remove" href=""><?php _e( 'Remove' , 'one-page' ); ?></a>
					</div>

					<a href="#uploadimage" class="layers-image-upload-button button layers-button btn-full <?php echo ( $has_image ) ? 'layers-has-image' : '' ; ?>"
						data-title="<?php _e( 'Select an Image' , 'one-page' ); ?>"
						data-button_text="<?php _e( 'Use Image' , 'one-page' ); ?>">
						<?php echo ( isset( $input->button_label ) ? $input->button_label : __( 'Choose Image' , 'one-page' ) ); ?>
					</a>
			...
			case 'upload' : ?>
				<section class="layers-file-container <?php if( isset( $input->value ) && NULL != $input->value ) echo 'layers-has-file'; ?>">
					...
					<a href="#choosefile" class="layers-regular-uploader layers-button btn-medium btn-full"
						data-title="<?php echo ( isset( $input->button_label ) ? esc_attr( $input->button_label ) : __( 'Choose File' , 'one-page' ) ); ?>"
						data-button_text="<?php _e( 'Use File' , 'one-page' ); ?>">
						<?php echo ( isset( $input->button_label ) ? $input->button_label : __( 'Choose File' , 'one-page' ) ); ?>
					</a>

					<a href="#removefile" class="<?php if( !isset( $input->value ) ) echo 'hide'; ?> layers-image-remove layers-file-remove">
						<?php _e( 'Remove' , 'one-page' ); ?>
					</a>
				...
				<div class="layers-media-controller" id="<?php echo esc_attr( $input->id ); ?>-controller">
					...
					<div class="layers-controller-elements">
						...
						<div class="layers-content <?php if( 'image' == $input_type ) echo 'section-active'; ?>">
							<div class="layers-form-item">
								<div class="layers-image-uploader layers-animate layers-push-bottom">
									<!-- Remove button -->
									<a class="layers-image-remove <?php if( !isset( $input->value->image ) ) echo 'l_admin-hide'; ?>" href=""><?php _e( 'Remove' , 'one-page' ); ?></a>

									<!-- Instructions -->
									<p <?php if( isset( $input->value->image ) ) echo 'class="l_admin-hide"'; ?>>
										<?php printf( __( 'Drop a file here or %s' , 'one-page' ) , '<a href="#">select a file.</a>' ); ?>
									</p>
							...
							<div class="layers-row">
								...
								<p class="layers-form-item">
									<label><?php _e( 'Background Color' , 'one-page' ); ?></label>
									<?php echo $this->input(
										array(
											'type' => 'color',
									...
									<li class="layers-checkbox">
										<?php echo $this->input(
											array(
												...
												'label' => __( 'Darken to improve readability' , 'one-page' )
											)
										); ?>
									</li>
									<li class="layers-checkbox">
										<?php echo $this->input(
											array(
												...
												'label' => __( 'Tile Background' , 'one-page' )
											)
										); ?>
									</li>
									<li class="layers-checkbox">
										<?php echo $this->input(
											array(
												...
												'label' => __( 'Fixed Background' , 'one-page' )
											)
										); ?>
									</li>
								</ul>
						...
						<div class="layers-content <?php if( 'video' == $input->value->type ) echo 'section-active'; ?>">
							<p class="layers-form-item">
								<label><?php _e( 'Enter your .mp4 link' , 'one-page' ); ?></label>
								<?php echo $this->input(
									array(
										'type' => 'upload',
										'name' => $input->name . '[mp4]' ,
							...
							<p class="layers-form-item">
								<label><?php _e( 'Enter your .ogv link' , 'one-page' ); ?></label>
								<?php echo $this->input(
									array(
										'type' => 'upload',
										'name' => $input->name . '[ogv]' ,
					...
					<div class="layers-form-collection-content">
						...
						<div class="layers-row">
							<div class="layers-form-item layers-column layers-span-5">
								...
								<?php echo $this->input(
									array(
										...
										'options' => array(
											'custom'            => __( 'Custom', 'one-page' ),
											'post'              => __( 'Existing Content', 'one-page' ), // Page, Post, Custom Post Type
											// 'post_type_archive' => __( 'Post Archives (incl. Custom Post Types)', 'one-page' ),
											// 'taxonomy_archive'  => __( 'Taxonomy Archives (incl. Custom Taxonomies)', 'one-page' ),
										),
										'value' => ( isset( $input->value->link_type ) ) ? $input->value->link_type : NULL,
										'class' => 'layers-text',
										'data' => array(
											'tip' => __( 'Choose Link Type', 'one-page' ),
										),
									)
								); ?>
							</div>
							<div class="layers-form-item layers-column layers-span-7 layers-link-field-holder">
								...
								<div class="layers-form-item layers-link-field layers-link-field-link_type_custom" data-show-if-selector= "#<?php echo "{$input->id}-link_type"; ?>" data-show-if-value="custom" data-tip="<?php _e( 'Choose Your Link', 'one-page' ) ?>">
									<label>
										<?php _e( 'Custom', 'one-page' ) ?>
									</label>
									<?php echo $this->input(
										array(
											'type' => 'text',
											...
											'placeholder' => __( 'http://' , 'one-page' ),
											'value' => ( isset( $input->value->link_type_custom ) ) ? $input->value->link_type_custom : NULL,
											'class' => 'layers-text',
											'data' => array(
												'tip' => __( 'Link', 'one-page' ),
											),
										)
									); ?>
								</div>
								<div class="layers-form-item layers-link-field layers-link-field-link_type_post" data-show-if-selector= "#<?php echo "{$input->id}-link_type"; ?>" data-show-if-value="post" data-tip="<?php _e( 'Choose Your Link', 'one-page' ) ?>">
									<label>
										<?php _e( 'Post/Page', 'one-page' ) ?>
									</label>
									<?php
									...
									echo $this->input(
										array(
											...
											'placeholder' => __( '-- Choose --' , 'one-page' ),
								...
								<div class="layers-form-item layers-link-field layers-link-field-link_type_post_type_archive" data-show-if-selector= "#<?php echo "{$input->id}-link_type"; ?>" data-show-if-value="post_type_archive" data-tip="<?php _e( 'Choose Your Link', 'one-page' ) ?>">
									<label>
										<?php _e( 'Post Archives', 'one-page' ) ?>
									</label>
									<?php echo $this->input(
										array(
											'type' => 'hidden',
								...
								<div class="layers-form-item layers-link-field layers-link-field-link_type_taxonomy_archive" data-show-if-selector= "#<?php echo "{$input->id}-link_type"; ?>" data-show-if-value="taxonomy_archive" data-tip="<?php _e( 'Choose Your Link', 'one-page' ) ?>">
									<label>
										<?php _e( 'Taxonomy Archives', 'one-page' ) ?>
									</label>
									<?php echo $this->input(
										array(
											'type' => 'hidden',
							...
							<div class="layers-form-item layers-column layers-span-12">
								...
								<?php echo $this->input(
									array(
										...
										'placeholder' => __( 'e.g. "Read More"' , 'one-page' ),
