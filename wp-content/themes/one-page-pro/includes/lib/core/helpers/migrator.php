...
//				),
//			'contact-page' => array(
//					'title' => __( 'Contact Page' , 'one-page' ),
...
//				),
//			'landing' => array(
//					'title' => __( 'Landing Page' , 'one-page' ),
...
//				),
//			'lookbook-page' => array(
//					'title' => __( 'Lookbook Page' , 'one-page' ),
...
//					'title' => __( 'Portfolio Page' , 'one-page' ),
...
';
	...
	*/
	...
	public function do_ajax_duplicate(){
		...
		$pageid = layers_create_builder_page( esc_attr( $_POST[ 'post_title' ] . ' ' . __( '(Copy)' , 'one-page' ) ) );
	...
	public function create_builder_page_from_preset(){
		...
		*/
		...
		if( isset( $_POST[ 'post_title' ] )  ){
			...
			$page_title = __( 'Home Page' , 'one-page' );
		}
		...
		foreach( $import_data[ 'widget_data' ] as $sidebar_id => $sidebar_data ) {
			...
			if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
				...
				if( $new_post_id ) {
					...
					$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)' , 'one-page' );
				}
			} else {
				$sidebar_available = false;
				$use_sidebar_id = 'wp_inactive_widgets'; // add to inactive if sidebar does not exist in theme
				...
				*/
				...
				if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message = __( 'Widget already exists' , 'one-page' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = __( 'Imported' , 'one-page' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = __( 'Imported to Inactive' , 'one-page' );
					}

				}
				// Result for widget instance
				$results[$sidebar_id]['widgets'][$widget_instance_id]['name'] = isset( $available_widgets[$id_base]['name'] ) ? $available_widgets[$id_base]['name'] : $id_base; // widget name or ID if name not available (not supported by site)
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = isset( $widget->title ) ? $widget->title : __( 'No Title' , 'one-page' ); // show "No Title" if widget instance is untitled
