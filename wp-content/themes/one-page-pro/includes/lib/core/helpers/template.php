...
if( !function_exists( 'layers_bread_crumbs' ) ) {
	function layers_bread_crumbs( $wrapper = 'nav', $wrapper_class = 'bread-crumbs', $seperator = '/' ) {
		...
		<<?php echo $wrapper; ?> class="<?php echo esc_attr( $wrapper_class ); ?>">
			<ul>
				<?php /* Home */ ?>
				<li><a href="<?php echo home_url(); ?>"><?php _e('Home' , 'one-page' ); ?></a></li>
				...
				*/
				...
				<?php } elseif( function_exists('is_shop') && ( is_post_type_archive( 'product' ) || ( get_post_type() == "product") ) ) { ?>
					...
					<?php if( function_exists( 'woocommerce_get_page_id' )  && '-1' != woocommerce_get_page_id('shop') ) { ?>
						...
						<?php } ?>
					<?php } else { ?>
						<li><a href="#"><?php _e( 'Shop' , 'one-page' ); ?></a></li>
					<?php }
	...
	function layers_get_page_title() {
		...
		if(!empty($parentpage) && !is_search()) {
			...
			if( function_exists( 'woocommerce_get_page_id' )  && -1 != woocommerce_get_page_id('shop') ) {
				...
				$title_array['title' ] = __( 'Shop' , 'one-page' );
			}
		} elseif( is_page() ) {
			...
			$title_array['title'] = __( 'Search' , 'one-page' );
			...
			$title_array['title' ] = sprintf( __( 'Daily Archives: %s' , 'one-page' ), get_the_date() );
		} elseif ( is_month() ) {
			$title_array['title' ] = sprintf( __( 'Monthly Archives: %s' , 'one-page' ), get_the_date( _x( 'F Y', 'monthly archives date format' , 'one-page' ) ) );
		} elseif ( is_year() ) {
			$title_array['title' ] = sprintf( __( 'Yearly Archives: %s' , 'one-page' ), get_the_date( _x( 'Y', 'yearly archives date format' , 'one-page' ) ) );
		} elseif( is_single() ) {
			$title_array['title' ] = get_the_title();
		} else {
			$title_array['title' ] = __( 'Archives' , 'one-page' );
		}
...
if ( ! function_exists( 'layers_show_html5_video' ) ) {
	function layers_show_html5_video( $src = NULL , $width = 490 ) {
		...
		<video width="<?php echo $width;?>" height="auto" controls>
			...
			<?php _e( 'Your browser does not support the video tag.' , 'one-page' ); ?>
		</video>
<?php }
...
if( !function_exists( 'layers_read_more_action' ) ) {
	function layers_read_more_action() {
		?>
		<a href="<?php the_permalink(); ?>" class="button"><?php echo apply_filters( 'layers_read_more_text', __( 'Read More' , 'one-page' ) ); ?></a>
		<?php
	}
}
