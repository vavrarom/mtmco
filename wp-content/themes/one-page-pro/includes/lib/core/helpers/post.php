        ...
        foreach ($display as $meta) {
            switch ($meta) {
                ...
                case 'categories' :
                    ...
                    $meta_to_display[] = '<span class="meta-item meta-category"><i class="l-folder-open-o"></i> ' . implode(__(', ', 'one-page'), $categories) . '</span>';
                    break;
                case 'tags' :
                    $tags = '';
                    ...
                    $meta_to_display[] = '<span class="meta-item meta-tags"><i class="l-tags"></i> ' . implode(__(', ', 'one-page'), $tags) . '</span>';
                    break;
                    break;
            } // switch meta
        } // foreach $display
...
if (!function_exists('layers_get_the_author')) {
    ...
    function layers_get_the_author() {
        return sprintf(__('<a href="%1$s" title="%2$s" rel="author">%3$s</a>', 'one-page'), esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_attr(sprintf(__('View all posts by %s', 'one-page'), get_the_author())), esc_attr(get_the_author())
        );
    }
...
if (!function_exists('layers_comment')) {
        ...
        <?php if (2 < $depth && isset($GLOBALS['lastdepth']) && $depth != $GLOBALS['lastdepth']) { ?>
            ...
            <div <?php comment_class('content well'); ?> id="comment-<?php comment_ID(); ?>">
                <div class="avatar push-bottom clearfix">
                    <?php edit_comment_link(__('(Edit)', 'one-page'), '<small class="pull-right">', '</small>') ?>
                    <a class="avatar-image" href="">
                        <?php echo get_avatar($comment, $size = '70'); ?>
                    </a>
                    <div class="avatar-body">
                        <h5 class="avatar-name"><?php echo get_comment_author_link(); ?></h5>
                        <small><?php printf(__('%1$s at %2$s', 'one-page'), get_comment_date(), get_comment_time()) ?></small>
                    </div>
                </div>

                <div class="copy small">
                    <?php if ($comment->comment_approved == '0') : ?>
                        <em><?php _e('Your comment is awaiting moderation.', 'one-page') ?></em>
                        <br />
                    <?php endif; ?>
                    <?php comment_text() ?>
    ...
    if (!function_exists('layers_backup_builder_pages')) {
        ...
        function layers_backup_builder_pages() {
            ...
            if (!check_ajax_referer('layers-backup-pages', 'layers_backup_pages_nonce', false))
                die('You threw a Nonce exception'); // Nonce

            if (!isset($_POST['pageid']))
                wp_die(__('You shall not pass', 'one-page'));
    ...
    if (!function_exists('layers_filter_admin_pages')) {
        ...
        function layers_filter_admin_pages() {
            ...
            if ('page' == $typenow && isset($_GET['filter']) && 'one-page' == $_GET['filter']) {
                set_query_var(
                        'meta_query', array(
                    'relation' => 'AND',
                    array(
        ...
        function layers_edit_layout_admin_menu() {
            ...
            if (!is_admin() && version_compare($wp_version, '4.2', '<=')) {
                ...
                $args = array(
                    ...
                    'title' => '<span class="ab-icon"></span><span class="ab-label">' . __('Edit Layout', 'one-page') . '</span>',
...
//    add_action('admin_print_styles-post.php', 'layers_add_builder_edit_button_css');
    ...
    if (!function_exists('layers_post_featured_media')) {
        ...
        function layers_post_featured_media($args = array()) {
            ...
            $post_meta = get_post_meta($postid, 'one-page', true);
