...
class Layers_Custom_Meta {
	...
	*/
	...
	public function __construct() {
		...
		$meta_dir = '/includes/lib/core/meta/';
	...
	*/
	...
	public function admin_enqueue_scripts(){
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/includes/lib/core/meta/js/meta.js' ,
			array(
				'backbone',
				'jquery',
				'wp-color-picker'
				...
				<div  class="postbox l_admin-push-top">
					<div class="l_admin-section-title l_admin-no-push-bottom l_admin-content">
						<div class="l_admin-heading">
							<?php ( 'auto-draft' == $post->post_status ? _e( 'Your page is almost ready.' , 'one-page' ) : _e( 'Your page is ready.' , 'one-page' ) ); ?>
						</div>
						<p class="l_admin-excerpt">
							<?php ( 'auto-draft' == $post->post_status ? _e( 'Click the Start button below to set this page up for Layers.' , 'one-page' ) : _e( 'You can drag and drop widgets, edit content and tweak the design. Click the button below to see your page come to life.' , 'one-page' ) ); ?>
						</p>
					</div>
					<div class="l_admin-button-well clearfix">
						<a href="<?php echo admin_url() . 'customize.php?url=' . esc_url( get_the_permalink() ); ?>" class="button btn-massive btn-primary btn-full <?php echo ( 'auto-draft' == $post->post_status ? 'disable' : '' ); ?>" id="<?php echo ( isset( $post->ID ) ? 'builder-button-' . $post->ID : 'builder-button-' . rand(0,1) ); ?>">
							<?php ( 'auto-draft' == $post->post_status ? _e( 'Start' , 'one-page' ) : _e( 'Edit Your Page' , 'one-page' ) ); ?>
						</a>
					</div>
				</div>
				...
				<div class="l_admin-row">
					...
					<div class="l_admin-column l_admin-span-4 postbox l_admin-content">
						<div class="l_admin-section-title l_admin-tiny">
							<h4 class="l_admin-heading"><?php _e( 'Export Layout', 'one-page' ); ?></h4>
							<p class="l_admin-excerpt"><?php _e( 'Export your layout to a <code>.json</code> file which you can use to upload to another site.', 'one-page' ); ?></p>
						</div>
						<a href="?post=<?php echo get_the_ID(); ?>&amp;action=edit&amp;layers-export=1" class="button"><?php _e( 'Export', 'one-page' ); ?></a>
					</div>

					<div class="l_admin-column l_admin-span-4 postbox l_admin-content">
						<div class="l_admin-section-title l_admin-tiny">
							<h4 class="l_admin-heading"><?php _e( 'Import Layout', 'one-page' ); ?></h4>
							<p class="l_admin-excerpt"><?php _e( 'Upload a layout file (eg. <code>' . $post->post_name . '.json</code>) by clicking the button below.', 'one-page' ); ?></p>
						</div>
						<button class="button" id="layers-page-import-button" data-post-id="<?php echo get_the_ID(); ?>" data-title="Upload .json" data-button_text="Upload &amp; Import"><?php _e( 'Upload &amp; Import', 'one-page' ); ?></button>
					</div>

					<div class="l_admin-column l_admin-span-4 postbox l_admin-content">
						<div class="l_admin-section-title l_admin-tiny">
							<h4 class="l_admin-heading"><?php _e( 'Duplicate Layout', 'one-page' ); ?></h4>
							<p class="l_admin-excerpt"><?php _e( 'Easily duplicate your layout, settings, text and images in order to get started quickly with a new page.', 'one-page' ); ?></p>
						</div>
						<button href="" class="button" id="layers-page-duplicate-button" data-post-id="<?php echo get_the_ID(); ?>"><?php _e( 'Duplicate', 'one-page' ); ?></button>
					</div>
	...
	*/
	...
	function inline_page_builder_button($actions,$post) {
		...
		if ( $can_edit_post && 'builder.php' == get_page_template_slug( $post->ID ) ) {
			$actions['builder'] = '<a href="' . admin_url( 'customize.php?url=' . esc_url( get_the_permalink() ) ) . '" title="' . esc_attr( __( 'Edit Layout' , 'one-page' ) ) . '">' . __( 'Edit Layout' , 'one-page' ) . '</a>';
		}
	...
	*/
	public function page_builder_button_pointer( $pointers ){
		...
		$pointers[ LAYERS_THEME_SLUG . '-builder-button-pointer-' . $post->ID ] = array(
					...
					'title'		=> __( 'Build Your Page' , 'one-page' ),
					'content'	=> __( 'Use the' . LAYERS_THEME_TITLE . ' page builder to build a beautiful, dynamic page.' , 'one-page' ),
				);
