...
class Layers_Customizer {
    ...
    */
    ...
    public function init() {
		...
		$controls_dir = '/includes/lib/core/customizer/controls/';
	...
	*/
	...
	public function admin_enqueue_scripts(){
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/includes/lib/core/customizer/js/customizer.js' ,
			array(
				'customize-controls',
				'wp-color-picker'
			),
	...
	*/
	...
	public function customizer_preview_enqueue_scripts(){
		...
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/includes/lib/core/customizer/js/customizer-preview.js',
		...
		wp_enqueue_style(
			LAYERS_THEME_SLUG . '-admin-customizer-preview',
			get_template_directory_uri() . '/includes/lib/core/customizer/css/customizer-preview.css',
			array(),
			LAYERS_VERSION
		);
	}
	...
	*/
	...
	public function admin_print_styles(){
		...
		wp_enqueue_style(
			LAYERS_THEME_SLUG . '-admin-customizer',
			get_template_directory_uri() . '/includes/lib/core/customizer/css/customizer.css',
			array(),
			LAYERS_VERSION
		);
	}
	...
	public function render_builder_page_dropdown(){
		...
		if( $layers_pages ){
			...
			<div class="layers-customizer-pages-dropdown">
				<select>
					<option value="init"><?php _e( 'Builder Pages:' , 'one-page' ) ?></option>
...
<!--		<div id="customize-controls-layers-actions">
			...
			<ul class="layers-customizer-nav">
				<li>
					<span class="customize-controls-layers-button customize-controls-layers-button-dashboard-main" title="<?php esc_html( _e( 'Layers Dashboard' , 'one-page' ) ) ?>" href="<?php echo admin_url( 'admin.php?page=layers-add-new-page' ); ?>"></span>
					<ul>
						<?php
						// Construct the Layers Customizer Menu
						$layers_customizer_menu = array(
							'preview' => array(
								'text'			=> __( 'View this page' , 'one-page' ),
								'link'			=> '#',
								'icon_class'	=> 'icon-display',
								'target'		=> '_blank',
							),
							'add-new-page' => array(
								'text'			=> __( 'Add new Layers page' , 'one-page' ),
								'link'			=> admin_url( 'admin.php?page=layers-add-new-page' ),
								'icon_class'	=> 'dashicons dashicons-plus',
							),
							'dashboard' => array(
								'text'			=> __( 'Layers Dashboard' , 'one-page' ),
